...
O componente `Button` suporta propriedades de acessibilidade para melhorar a experiência de usuários que utilizam leitores de tela.

### Aria Label

Utilize a propriedade `ariaLabel` para fornecer uma descrição mais detalhada do botão para leitores de tela.

<Canvas sourceState={SourceState.OPEN}>
  <Story story={WithAriaLabel} />
</Canvas>
...
O componente `FreudButtonComponent` possui as seguintes propriedades:

#### Propriedades Básicas
- **`color`**: Define a cor do botão
  - Tipo: `'primary' | 'secondary' | 'ghost'`
  - Padrão: `'primary'`

- **`bgColor`**: Define se o botão deve ter cor de fundo
  - Tipo: `boolean`
  - Padrão: `false`
...
- **`label`**: Define o texto do botão
  - Tipo: `string`
  - Padrão: `''`

- **`iconPos`**: Define a posição do ícone
  - Tipo: `'left' | 'right'`
  - Padrão: `'left'`

- **`fullWidth`**: Define se o botão deve ocupar toda a largura disponível
  - Tipo: `boolean`
  - Padrão: `false`

#### Propriedades de Acessibilidade
- **`ariaLabel`**: Fornece uma descrição mais detalhada do botão para leitores de tela
  - Tipo: `string`
  - Padrão: `''`
