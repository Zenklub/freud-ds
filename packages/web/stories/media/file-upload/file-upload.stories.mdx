...
import { FreudFileUploadComponent, FreudFileUploadModule } from '@freud-ds/web-components';
import { FreudHeaderComponent } from '../../header/header.component';
import {
  Default,
  BGColor,
  BasicUi
} from './FileUpload.stories.ts';

<Meta
  decorators={[
    moduleMetadata({imports: [FreudFileUploadModule]})
  ]}
  parameters={{
    layout: 'centered',
    backgrounds: {
...
```html
<freud-file-upload [name]="'file[]'" [url]="'https://api.com/upload'"></freud-file-upload>
```
...
```html
<freud-file-upload name="myfile[]" customUpload="true" (uploadHandler)="myUploader($event)"></freud-file-upload>
```
```ts
  myUploader(event) {
